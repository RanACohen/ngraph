...
#define IR_ASSERT(x) if (!(x)) THROW_IE_EXCEPTION << "Assert failed for " #x << ": "
    ...
    typedef InferenceEngine::CNNLayer::Ptr     IELayer;
    typedef InferenceEngine::DataPtr           OutputPort;

    typedef InferenceEngine::Blob           IETensor;
    typedef InferenceEngine::TBlob<float>   IEFP32Tensor;


    //    struct Matrix
    //    {
    //        static IETensor::Ptr read(const char* fileName, uint32_t rows, uint32_t cols);
    //    };
...
struct DelayObj {
    ...
    inline void operator >> (
        const InferenceEngine::CNNLayerPtr &lhs,
        const InferenceEngine::CNNLayerPtr &rhs) {
        lhs->outData[0] >> rhs;
    }

    inline IETensor::Ptr readBlobFromFile(const std::string &file) {
        auto fs = FileUtils::fileSize(file);
        if (fs <= 0) THROW("blob file ") << file << " not found or empty";
        auto ret = IEFP32Tensor::Ptr(new IEFP32Tensor(InferenceEngine::Precision::FP32, InferenceEngine::C,
        { static_cast<size_t>(fs / sizeof(float)) }));
        ret->allocate();
        FileUtils::readAllFile(file, ret->data(), fs);
        return ret;
    }

    inline IETensor::Ptr readBlobFromFile(const std::string &file, const TensorDims &dims, InferenceEngine::Layout l) {
